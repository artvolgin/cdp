...
Let $X, Y$ be sets of items (e.g., certain hazards), $X \Rightarrow Y$ an association rule, $T$ a set of paths (e.g., hazard - action - co-benefit).

1. Support

Support indicator reflects the frequency of item's appearance in the database

$$ \mathrm{supp}(X) = \frac{|\{t \in T; X \subseteq t\}|}{|T|}  $$
2. Confindence

Confidence indicator points on how frequent a rule has been established to be true.


$$ {conf}(X \Rightarrow Y) = \mathrm{supp}(X \cup Y) / \mathrm{supp}(X) $$
3. Lift

Lift indicator potrays the ratio of the observed support to the expected support under the independence of $X$ and $Y$.
...
df_additional_info <- rio::import("df_cities_info.additional.xlsx")
...
# Graphing
mean_KPI_2019 <- cities19_20_KPIs_year %>% group_by(year) %>%
  summarise(mean_KPI=mean(KPI_total)) %>% filter(year==2019)
mean_KPI_2020 <- cities19_20_KPIs_year %>% group_by(year) %>%
  summarise(mean_KPI=mean(KPI_total)) %>% filter(year==2020)

cities19_20_KPIs_year %>%
  ggplot(aes(x = KPI_total, fill = factor(year))) +
  geom_density(alpha=.3) + 
  labs(x = "Total KPI", y = "Density", fill="") + 
  scale_fill_manual(values=c('darksalmon','deepskyblue2')) + 
  ggtitle("KPI Distributions for 2019 and 2020") + 
  geom_vline(aes(xintercept=mean(mean_KPI_2019$mean_KPI, na.rm=T)),   # Ignore NA values for mean
               color="darksalmon", linetype="dashed", size=1) + 
  geom_vline(aes(xintercept=mean(mean_KPI_2020$mean_KPI, na.rm=T)),   # Ignore NA values for mean
               color="deepskyblue2", linetype="dashed", size=1) + 
  theme_classic() +
  theme(plot.title = element_text(size=18, face="bold", hjust=0.5),
        legend.position="bottom") + 
  annotate("text", label = "Mean KPI\n2019", x = 27.5, y = 0.01, size = 4, colour = "darksalmon") + 
  annotate("text", label = "Mean KPI\n2020", x = 39, y = 0.01, size = 4, colour = "deepskyblue2")

# TODO: Add mean values near the lines
...
```{r}
# Heatmap
cities19_20_KPIs_top20[,c(1,3,2,4)] %>%
  kbl() %>%
...
cities20_KPIs_plot <- cities20_KPIs %>% dplyr::select(id, KPI_total)
cities20_KPIs_plot <- cities20_KPIs_plot %>% left_join(df_additional_info) %>%
  mutate(population=population/1e6)

ggplot(cities20_KPIs_plot, aes(x=gdp_per_capita, y=KPI_total)) + 
  geom_point(aes(size=population, color = mean_temp)) +
  geom_smooth(method = lm, se = T, color = "darkgrey", size=0.5) +
  scale_color_gradient(low="deepskyblue2", high="darksalmon") +
  labs(x = "GDP per capita, $", y = "Total KPI",
       size="Population (millions)", col="Mean Annual Temperature, °C") + 
  ggtitle("Total KPI and GDP per capita") + 
  theme_classic() +
  theme(plot.title = element_text(size=18, face="bold", hjust=0.5),
        legend.position="bottom")
...
```{r fig.height=10, fig.width=10}
colnames(slacks_eff_high_gdp) <- gsub("_", " ", colnames(slacks_eff_high_gdp))
colnames(slacks_eff_low_gdp) <- gsub("_", " ", colnames(slacks_eff_low_gdp))

pheatmap(slacks_eff_high_gdp, cluster_rows=T, cluster_cols=F, angle_col = "45",
color = colorRampPalette(c("chartreuse3", "white",  "brown2"))(100),
border_color="white", display_numbers=F, legend=F, main="Slacks of cities with above average GDP")

pheatmap(slacks_eff_low_gdp, cluster_rows=T, cluster_cols=F, angle_col = "45",
color = colorRampPalette(c("chartreuse3", "white",  "brown2"))(100),
border_color="white", display_numbers=F, legend=F)
```
...
# Set vertex size
vertex_size = log(get.vertex.attribute(G2)$in_deg + 2)*3
vertex_size <- ifelse(vertex_size<1.5, 1.5, vertex_size)

# Set vertex color
gdp <- get.vertex.attribute(G2)$gdp_per_capita
in_degree <- get.vertex.attribute(G2)$in_deg
vertex_color <- ifelse( (gdp >= mean(gdp) & in_degree>=3), 'deepskyblue2',
                        ifelse((gdp >= mean(gdp) & in_degree<3), adjustcolor('deepskyblue2', alpha=0.5),
                        ifelse((gdp < mean(gdp) & in_degree>=3), 'darksalmon',
                        adjustcolor('darksalmon', alpha=0.5) )))

# Set vertex label
vertex_label <- get.vertex.attribute(G2)$name
vertex_label_temp <- ifelse(in_degree>=3, vertex_label, NA)
vertex_label <- dplyr::recode(vertex_label,
                              "City of Boulder"="Boulder",
                              "City of Greater Sudbury / Grand Sudbury"="Greater Sudbury",
                              "Ayuntamiento de Hermosillo"="Hermosillo",
                              "City of Bloomington"="Bloomington",
                              "City Government of Davao"="Davao",
                              "Alcaldia de Pereira"="Pereira",
                              "Município de Guimarães"="Guimarães",
                              "Município de Braga"="Braga",
                              "Municipalidad de Providencia"="Providencia",
                              "City of Encinitas, CA"="Encinitas",
                              "Tagum City"="Tagum")
vertex_label <- ifelse(in_degree>=3, vertex_label, NA)

# Set vertex frame color
vertex_frame_color <- ifelse(in_degree>=3, adjustcolor('black', alpha=0.5), "lightgrey")

# Sed edge color
edgelist <- data.frame(as_edgelist(G2))
edge_color <- ifelse(edgelist$X2 %in% c(na.omit(unique(vertex_label_temp, na.rm=T))),
                     adjustcolor('darkgrey', alpha=0.5), adjustcolor('lightgrey', alpha=0.3))

# Set edge width
edge_width <- E(G2)$weight

# # Set vertex label color
# vertex_label_color <- ifelse( (gdp >= mean(gdp) & in_degree>=3), 'midnightblue',
#                         ifelse((gdp >= mean(gdp) & in_degree<3), NA,
#                         ifelse((gdp < mean(gdp) & in_degree>=3), 'brown4',
#                         NA )))

set.seed(12)
plot(G2,
     edge.arrow.size = 0.3,
     # layout=layout.lgl, # <<< OK
     layout=layout.mds, # <<< OK2
     ...
     vertex.label.dist=0.75, vertex.label.degree=-pi/2, 
     vertex.frame.color=vertex_frame_color,
     # label.dist = 0,
     # vertex.label.dist = 1,
     # vertex.label.degree = 30,
     # edge.color=edge_color,
     edge.color=adjustcolor('lightgrey', alpha=0.5),
