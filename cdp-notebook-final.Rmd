...
##################################################################
#Proportion of actions addressed in hazards with social impact for vulnerable populations 
##################################################################

# Selecting all hazards reported
Q2.1 <- cities20 %>% select(id, org, qstn, coln, rown, resp) %>% 
  filter(qstn == '2.1' & coln %in% c(1,5)) %>% select(-qstn)%>% arrange(org, coln, rown)
Q2.1$resp <- as.character(Q2.1$resp)
...
# Selecting reports with vulnerable populations and filtering only non-missing values
Q2.1_7 <- cities20 %>% select(id, org, qstn, coln, rown, resp) %>% 
  filter(qstn == '2.1' & coln == 5) %>% select(-qstn) %>% arrange(org, coln, rown) %>%
  filter(!resp == '')
...
# Defining the amount of social_impact addressed
df_social_impact_addressed <- Q2.1_Q3.0_2_Q2.1_7 %>% group_by(id, org) %>%
  filter(!is.na(resp.y)) %>% # filtering those who did not report on social_impact for a certain hazard
  summarise(social_impact_addressed = n(), .groups = 'drop')

# Defining the amount of social_impact in total
df_social_impact_total <- Q2.1_7 %>% group_by(id, org) %>%
  summarise(social_impact_total = n(), .groups = 'drop')

# Addressed per total
df_social_impact_addressed_per_total <- df_social_impact_addressed %>%
  left_join(df_social_impact_total, by = c('id', 'org')) %>%
  filter(!is.na(social_impact_total)) %>% 
  mutate(social_impact_addressed_per_total = social_impact_addressed/social_impact_total) %>%
  select(id, org, social_impact_addressed_per_total)

# Merging with the selected variables so far
cities20_ <- plyr::join_all(list(cities20_, df_vuln_addressed_per_total,
                                 df_social_impact_addressed_per_total), by = c('id', 'org'), type='full')
...
Q5.0a <- emission_reduction_targets(cities20, '5.0a', 10, 9)
Q5.0b <- emission_reduction_targets(cities20, '5.0b', 8, 7)
Q5.0c <- emission_reduction_targets(cities20, '5.0c', 12, 11)
Q5.0d <- emission_reduction_targets(cities20, '5.0d', 10, 8)
...
# Merging with the selected variables so far
cities20_ <- plyr::join_all(list(cities20_, Q5.0abcd, Q5.4_1, Q5.4_4, Q5.4_9), by = c('id', 'org'), type='full')
#length(unique(cities20_$id))
...
# A lot of missing values and variance is poor -> drop
...
# Removing cities with >10 missing values
cities20_items_only_2 <- cities20_items_only_ %>% filter(! org %in% as.character(row_na[row_na$row_na_sum > 10, 'cities20_items_only_$org']))
...
# Imputing NA for 2020
###################################
library(Amelia)
# Setting bounds for imputations
max <- as.matrix(as.numeric(as.character(apply(cities20_items_only_2, 2 , max, na.rm=TRUE))))
max <- as.matrix(max[3:nrow(max),])
# Running imputation
set.seed(123)
amelia_res <- amelia(cities20_items_only_2, m = 10, idvars = c('id', 'org'),
                     bounds = cbind(3:53, 0, max))

imputed_data2 <- amelia_res$imputations$imp10
#summary(imputed_data2)

cities20_items_only_2 <- imputed_data2
###################################
```
...
subset <- c("3.2",	"3.1a",	"3.3",	"4.0",	"4.15",	"6.1a",
            ...
            "14.1",	"10.7",	"13.0",	"14.4",
            "14.5a", '2.1', '3.0', '10.5', '12.4',	"8.2", '0.4', '2.0')
...
# Number of sustainability goals
Q1.0a_1 <- count_rows(Q1.0a, 1, sustain_goals, year = 2019)
...
# Averaging for all assessments
Q2.0b_4 <- mean_for_multiple_responses(Q2.0b, 4, boundary_assess, year = 2019)
...
# Averaging by all documents
Q2.0b_8 <- mean_for_multiple_responses(Q2.0b, 8, vulnpop_assess, year = 2019)

###### Number of assessment publications and areas/sectors covered in total
Q2.0b_1 <- count_rows(Q2.0b, 1, n_assess, year = 2019)

###### Number of areas/sectors covered in total
Q2.0b_6 <- count_rows(Q2.0b, 6, areas_assess, year = 2019)
...
###### Number of documents
Q3.1a_1 <- count_rows(Q3.1a, 1, n_doc_adaptation, year = 2019)
...
Q3.1a_2_categories <- c('Energy',
                        ...
                        'Business and Financial Service',
                        'Social Services')
Q3.1a_2 <- count_rows(Q3.1a, 2, n_areas_adaptation_plan, select_from = T,
                      categories = Q3.1a_2_categories, year = 2019)
...
# Averaging values of a question about boundaries for all plans
Q3.1a_4 <- mean_for_multiple_responses(Q3.1a, 4, boundary_adaptation_plan, year = 2019)
...
# Averaging values of the question about a stage for all plans
Q3.1a_6 <- mean_for_multiple_responses(Q3.1a, 6, stage_adaptation_plan, year = 2019)
...
Q3.1a[Q3.1a$coln == 7,'resp'] <- ifelse(Q3.1a[Q3.1a$coln == 7,'resp'] == 'Standalone', 1,
                                 ...
                                 ifelse(Q3.1a[Q3.1a$coln == 7,'resp'] %in%  as.character(str_subset(Q3.1a[Q3.1a$coln == 7,'resp'], "^Other, please specify")), 2,
...
# Average type of a plan
Q3.1a_7 <- mean_for_multiple_responses(Q3.1a, 7, adaptation_plan_type, year = 2019)
...
# Averaging values of the question about local government's assessment for all plans
Q3.1a_8 <- mean_for_multiple_responses(Q3.1a, 8, localgov_assess_adaptation_plan, year = 2019)
...
# Merging with the selected variables so far
cities19_ <- plyr::join_all(list(cities19_, df_vuln_addressed_per_total,
                                 df_social_impact_addressed_per_total), by = c('id', 'org'), type='full')
...
Q3.0_1 <- count_rows(Q3.0, 1, n_hazards, year = 2019)
...
######  Number of actions
Q3.0_2 <- count_rows(Q3.0, 2, n_adaptation_actions, year = 2019)
...
# Averaging values of a question about status for all actions
Q3.0_4 <- mean_for_multiple_responses(Q3.0, 4, adaptation_action_status, year = 2019)

######  Number of co-benefits
Q3.0_5 <- count_rows(Q3.0, 5, n_adaptation_cobenefits, year = 2019)

######  Number of Sectors/areas adaptation actions apply to
#Q3.0_7 <- count_rows(Q3.0, 7, n_adaptation_actions_areas)

# Merging with the selected variables so far
cities19_ <- plyr::join_all(list(cities19_, Q3.0_2, Q3.0_4, Q3.0_5), by = c('id', 'org'), type='full')
#length(unique(cities19_$id))
...
######  Number of goals
Q3.2_1 <- count_rows(Q3.2, 1, n_goals, year = 2019)
...
###### Q5.0: % of target achieved and reduction target in metric tonnes
Q5.0a <- emission_reduction_targets(cities19, '5.0a', 10, 9)
Q5.0b <- emission_reduction_targets(cities19, '5.0b', 8, 7)
Q5.0c <- emission_reduction_targets(cities19, '5.0c', 12, 11)
Q5.0d <- emission_reduction_targets(cities19, '5.0d', 10, 8)
...
# Combining all target types and summarising if a city fell into 2 categories
Q5.0abcd <- rbind.data.frame(Q5.0a, Q5.0b, Q5.0c, Q5.0d, Q5_no_target) %>% group_by(id, org) %>% summarise_all(list(sum))
...
# Number of mitigation actions
Q5.4_1 <- count_rows(Q5.4, 1, n_mitigation_actions, year = 2019)
...
# Averaging values of a question about status for all actions
Q5.4_4 <- mean_for_multiple_responses(Q5.4, 4, mitigation_action_status, year = 2019)

######  Number of co-benefits
Q5.4_9 <- count_rows(Q5.4, 9, n_mitigation_cobenefits, year = 2019)

###### Estimated emissions reduction: sum for all actions 
#Q5.4_5 <- sum_for_multiple_responses(Q5.4, 5, estimated_emissions_reduction, year = 2019)

###### Energy savings (MWh): sum for all actions 
#Q5.4_6 <- sum_for_multiple_responses(Q5.4, 6, energy_savings, year = 2019)

###### Renewable energy production (MWh): sum for all actions 
#Q5.4_7 <- sum_for_multiple_responses(Q5.4, 7, renewable_energy_production, year = 2019)

# Merging with the selected variables so far
cities19_ <- plyr::join_all(list(cities19_, Q5.0abcd, Q5.4_1, Q5.4_4, Q5.4_9), by = c('id', 'org'), type='full')
#length(unique(cities19_$id))
...
######  Number of documents
Q5.5a_1 <- count_rows(Q5.5a, 1, n_doc_mitigation, year = 2019)
...
# Averaging values of a question about boundaries for all plans
Q5.5a_5 <- mean_for_multiple_responses(Q5.5a, 5, boundary_mitigation_plan, year = 2019)
...
# Averaging values of the question about a stage for all plans
Q5.5a_7 <- mean_for_multiple_responses(Q5.5a, 7, stage_mitigation_plan, year = 2019)
...
# Averaging values of the question about local government's assessment for all plans
Q5.5a_8 <- mean_for_multiple_responses(Q5.5a, 8, localgov_assess_mitigation_plan, year = 2019)
...
# Number of opportunities
Q6.0_1 <- count_rows(Q6.0, 1, n_opportunities, year = 2019)
...
# Number of collaboration areas
Q6.1a_1 <- count_rows(Q6.1a, 1, n_collab_areas, year = 2019)
...
# Number of unique project areas
Q6.2_1 <- count_rows(Q6.2, 1, n_project_areas, year = 2019)
...
# Averaging values of the question about a stage for all projects
Q6.2_3 <- mean_for_multiple_responses(Q6.2, 3, stage_project, year = 2019)
...
# Average percentage of renewable energy / electricity of total energy or electricity in target year
Q8.0a_8 <- mean_for_multiple_responses(Q8.0a, 8, percentage_renewable_energy, year = 2019)

# Average percentage of targets achieved
Q8.0a_9 <- mean_for_multiple_responses(Q8.0a, 9, renewable_energy_achieved, year = 2019)
...
# Average percentage of energy efficiency improvement in target year compared to base year levels
Q8.6a_7 <- mean_for_multiple_responses(Q8.6a, 7, percentage_energy_efficiency_improvement, year = 2019)

# Average percentage of target achieved
Q8.6a_8 <- mean_for_multiple_responses(Q8.6a, 8, energy_efficiency_achieved, year = 2019)
...
Q10.1 <- cities19 %>% select(id, org, qstn, coln, rown, resp) %>% 
  ...
  summarise(percentage_walking_cycling = ifelse(all(is.na(as.numeric(as.character(resp)))),
                                                NA, sum(as.numeric(as.character(resp)), na.rm = T)), .groups = 'drop')
# Adding cities with 'not applicable" responses
na_set_Q10.1 <- cities19 %>% filter(qstn == '10.1' & coln %in% c(5,6)) %>%
  filter(is.na(resp) | resp == "") %>% select(org)
Q10.1 <- rbind.data.frame(Q10.1, full_cities19 %>% filter(!org %in% unique(Q10.1$org) & !org %in% na_set_Q10.1$org) %>%
                          select(id, org) %>%
                          mutate(percentage_walking_cycling = 9999999)) %>%
      distinct(.keep_all = T)
...
Q10.5 <- cities19 %>% select(id, org, qstn, coln, rown, resp) %>% 
  ...
  summarise(eco_private_cars = ifelse(all(is.na(as.numeric(as.character(resp)))),
                                      NA, sum(as.numeric(as.character(resp)), na.rm = T)), .groups = 'drop')
# Adding cities with 'not applicable" responses
na_set_Q10.5 <- cities19 %>% filter(qstn == 'Q10.5' & coln == 1 & rown %in% 2:5) %>%
  filter(is.na(resp) | resp == "") %>% select(org)
Q10.5 <- rbind.data.frame(Q10.5, full_cities19 %>% filter(!org %in% unique(Q10.5$org) & !org %in% na_set_Q10.5$org) %>%
                          select(id, org) %>%
                          mutate(eco_private_cars = 9999999)) %>%
      distinct(.keep_all = T)
...
# Merging with the selected variables so far
#cities19_ <- plyr::join_all(list(cities19_, Q13.0), by = c('id', 'org'), type='full')
#length(unique(cities19_$id))
...
# Number of unique adaptation actions
Q14.4_2 <- count_rows(Q14.4, 2, n_water_adaptation_actions, year = 2019)
...
# Averaging values for all water adaptation actions
Q14.4_3 <- mean_for_multiple_responses(Q14.4, 3, water_adaptation_action_status, year = 2019)
...
# Number of documents
Q14.5a_1 <- count_rows(Q14.5a, 1, n_water_management, year = 2019)
...
# Averaging values of the question about a stage for all plans
Q14.5a_5 <- mean_for_multiple_responses(Q14.5a, 5, stage_water_management_plan, year = 2019)
...
cities19_items_only$target_metric_tonnes <- cities19_items_only$target_metric_tonnes/cities19_items_only$population
#cities19_items_only$estimated_emissions_reduction <- cities19_items_only$estimated_emissions_reduction/cities19_items_only$population
#cities19_items_only$energy_savings <- cities19_items_only$energy_savings/cities19_items_only$population
#cities19_items_only$renewable_energy_production <- cities19_items_only$renewable_energy_production/cities19_items_only$population
#cities19_items_only$waste_amount <- cities19_items_only$waste_amount/cities19_items_only$population

# Remove if NA >10
# Recalculating missing patterns
#row_na <- cbind.data.frame(cities19_items_only$org,  'row_na_sum' = rowSums(is.na(cities19_items_only))) %>% arrange(row_na_sum)

# Removing cities with >10 missing values
#cities19_items_only <- cities19_items_only %>% filter(! org %in% as.character(row_na[row_na$row_na_sum > 10, 'cities19_items_only$org']))

# Imputing NA for 2019
###################################
# Setting bounds for imputations
max <- as.matrix(as.numeric(as.character(apply(cities19_items_only, 2 , max, na.rm=TRUE))))
max <- as.matrix(max[3:nrow(max),])

# Running imputation
set.seed(123)
amelia_res <- amelia(cities19_items_only, m = 10, idvars = c('id', 'org'),
                     bounds = cbind(3:50, 0, max))

data_imputed <- amelia_res$imputations$imp9
#summary(data_imputed)

cities19_items_only <- data_imputed
###################################
...
for (i in colnames(cities19_norm)[3:ncol(cities19_norm)]){
...
for (i in colnames(cities20_norm)[3:ncol(cities20_norm)]){
...
cities20_norm$Social_Equity <- rowMeans(cities20_norm[,c('vulnpop_assess',
                                                         ...
                                                         'social_impact_addressed_per_total')])
...
Malmquist_output <- Malmquist_output[,c('org', 'MI', 'TC',
                                        'PECH', 'eff_19',
                                        'eff_20', 'gdp_group')]
colnames(Malmquist_output) <- c('Cities', 'MI', 'TC', 'PECH',
...
# Printing
kable(list(Malmquist_output_low_gdp, Malmquist_output_high_gdp),
